...
A simple unikernel anthropic service that calls anthropic (claude sonnet) api endpoints written in Rust

## Requirements

This service uses (openai schema) as its base, it can be configured to use the specific schema enabled service, along with the relevant api token
...
cd rust-ai-unikernel-anthropic-service
...
```
./target/release/ai-unikernel-anthropic-service
```
