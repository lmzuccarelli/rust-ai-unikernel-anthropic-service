...
use crate::api::schema::AnthropicResponse;
...
async fn process_post_call(data: Bytes) -> Result<String, Box<dyn std::error::Error>> {
    ...
    let response = client_response.bytes().await?;
    let chat_response: AnthropicResponse = serde_json::from_slice(&response)?;
    log::debug!(
        "[process_post_call] output tokens {}",
        chat_response.usage.output_tokens
    );
    log::debug!(
        "[process_post_call] inpput tokens {}",
        chat_response.usage.input_tokens
    );
    Ok(format!("\n{:?}\n", chat_response.content[0].text))
}
